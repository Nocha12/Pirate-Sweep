...
Emitter::Emitter(path spritePath, bool isBlending, bool isFallable)
	: isBlending(isBlending), started(false), currentCount(0), time(0), rotationSpeed(0), rotationOffset(0), isFallable(isFallable)
{
	...
	this->spritePath.push_back(spritePath);
}
...
void Emitter::createParticle()
{
	...
	Sprite* s = new Sprite( spritePath[random(0, spritePath.size() - 1)]);
	s->scale = Vec2( scale, scale );
	s->blending = isBlending;

	Particle p = {s, speed, 0, lifeTime, rotationSpeed, false, false, false};
...
void Emitter::update(float dt)
{
	...
	for_iter(iter, particles) {
		if (isFallable && !iter->isFollowing)
		{
			iter->gravity += 9.8 * dt + 50 * dt;
			iter->sprite->pos.y += iter->gravity;
			iter->sprite->pos += iter->speed*dt;
		}
	
		iter->lifeTime += dt;

		if (iter->lifeTime > 0.7)
			iter->isFollowPlayer = true;
